...
const CreateEvent = () => {
  ...
  const [credentials, setCredentials] = useState({eventname: '', eventdescription: '', eventdate: '', eventemail: '', eventPhone: '', eventaddress: ''})
  ...
  const handleSubmit = async(e) => {
    ...
    if (!credentials.eventname || !credentials.eventdescription || !credentials.eventdate || !credentials.eventemail || !credentials.eventPhone || !credentials.eventaddress) {
    ...
    const response = await fetch("http://localhost:5000/api/event/addevent",{
      ...
      body: JSON.stringify({eventname: credentials.eventname, eventdescription: credentials.eventdescription, eventdate: credentials.eventdate, eventemail: credentials.eventemail, eventPhone: credentials.eventPhone, eventaddress: credentials.eventaddress})  
    });
                  ...
                  <button className="btn-close" onClick={handleCloseForm}><FaRegTimesCircle /></button>
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      <div className="form-group">
                        ...
                        <textarea className="form-control" id="eventdescription" name="eventdescription" rows="3" value={credentials.eventdescription} onChange={handleInputChange} required></textarea>
                      </div>
                      <div className="form-group">
